...
// mode select (1: add, 0: subtract)
assign #1 mode_inv = ~mode;
xor #1 XOR0(XOR[0], mode_inv, B[0]);
xor #1 XOR1(XOR[1], mode_inv, B[1]);
xor #1 XOR2(XOR[2], mode_inv, B[2]);
xor #1 XOR3(XOR[3], mode_inv, B[3]);
xor #1 XOR4(XOR[4], mode_inv, B[4]);
xor #1 XOR5(XOR[5], mode_inv, B[5]);
xor #1 XOR6(XOR[6], mode_inv, B[6]);
xor #1 XOR7(XOR[7], mode_inv, B[7]);


// full adders (sum path + P/G generation)
no_Cout_adder Addr0(.A(A[0]), .B(XOR[0]), .Cin(mode_inv), .S(sum[0]), .P(P[0]), .G(G[0]));
...
eight_bit_carry_lookahead_generator CLAG(.Cin(mode_inv), .P(P), .G(G), .C({c_out, C}));
